...
Matrix4x4f::Matrix4x4f(float arr[]) : Matrix<float>::Matrix(arr, 4, 4) {}


Matrix4x4f Matrix4x4f::mul(Matrix4x4f& m1, Matrix4x4f& m2) {
	return Matrix<float>::mul(m1, m2);
}

Matrix4x4f Matrix4x4f::mul(Matrix4x4f& m1, float scalar) {
	return Matrix<float>::mul(m1, scalar);
}

void Matrix4x4f::setData(float arr[]) {
	for (int y = 0; y < mY; y++)
		for (int x = 0; x < mX; x++)
			mData[y * mX + x] = arr[y * mX + x];
}

Matrix4x4f::Matrix4x4f() : Matrix<float>(4, 4) {
	for (int i = 0; i < 16; i++)
		mData[i] = 0;
}
